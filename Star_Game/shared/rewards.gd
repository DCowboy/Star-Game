	...
	if number > 0:
		for each in range(number):
			...
			if item != null:
				...
				item_pos.x += int(rand_range(-1, 1) * (child.get_texture().get_width() * child.get_scale().x))
				item_pos.y += int(rand_range(-1, 1) * (child.get_texture().get_height() * child.get_scale().y))
